function dy = SCION_equations_jurassic(t,y)
...
global sensparams
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Load astronomical temperature forcing data
global T_ext_data
T_ext_data = load('T_ext_8Ma_4p8Ma.mat');  
global scenarioID

    %  scenarioID - 7 situation
    switch scenarioID
        case 'S1'
            % only volcanic
        case 'S2'
        ...
        case 'S6'
            % 8Ma + volcanism
        case 'S7'
            % 8Ma + 4.8Ma (no volcanism)
        otherwise
            error('Unknown scenario: %s', scenarioID);
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%% Scenario forcing structure
switch scenarioID
    case 'S1'  % only volcanic
      forcings.volcanics = [...
            -201,   1.19e13,   -20;
            -200,   1.19e13,   -20;
            -199.9, 0 , -5 ;
        ...
        C_volc = interp1(forcings.volcanics(:,1), forcings.volcanics(:,2), t_geol, 'linear', 0);
        delta_C_volc = interp1(forcings.volcanics(:,1), forcings.volcanics(:,3), t_geol, 'linear', 0);
        T_offset = 0;
    ...
    case 'S2'  % only 8Ma
        ...
        T_offset = interp1(T_ext_data.time8_Ma, T_ext_data.signal8, t_geol, 'linear', 0);
        % T_offset = (T_offset + 1) * 0.3;
        T_offset = T_offset * 0.2 + 0.8;
       
    case 'S3'  % 4.8Ma + volcanic
      forcings.volcanics = [...
            -201,   1.19e13,   -20;
            -200,   1.19e13,   -20;
            -199.9, 0 , -5 ;
        ...
        T_offset = interp1(T_ext_data.time48_Ma, T_ext_data.signal48, t_geol, 'linear', 0);
        % T_offset = (T_offset + 1) * 0.3;
        T_offset = T_offset * 0.2 + 0.8;

    case 'S4'  % 8Ma + 4.8Ma + volcanic
        forcings.volcanics = [...
            -201,   1.19e13,   -20;
            -200,   1.19e13,   -20;
            -199.9, 0 , -5 ;
        ...
        delta_C_volc = interp1(forcings.volcanics(:,1), forcings.volcanics(:,3), t_geol, 'linear', 0);
        T_ext_data = load('T_ext_8Ma_4p8Ma.mat');
        T_offset = interp1(T_ext_data.Time_Ma, T_ext_data.T_ext_K, t_geol, 'linear', 0);
        % T_offset = (T_offset + 1) * 0.3;
        T_offset = T_offset * 0.2 + 0.8;
    ...
    otherwise
        error('Undefined scenarioID.');
end

%%%%% Apply temperature-dependent scaling
F_silw_temp_adjust = exp(0.09 * T_offset);
F_flux = 1 + 0.1 * T_offset;    %%for flux
F_isotope = 1 - 0.25 * T_offset; %%only for carbon isotope
T_env = T_offset + 273;

%%%%% Volcanic isotopic flux
volc_iso_flux = C_volc * delta_C_volc;
...
%%%% overall weathering
silw = basw + granw ;
silw_J = silw * F_silw_temp_adjust; %%%%%%%%%%%%%
...
phosw = pars.k_phosw*( (pfrac_silw)*( silw/pars.k_silw )  +   (pfrac_carbw)*( carbw/pars.k_carbw ) +  (pfrac_oxidw)*(  oxidw/ pars.k_oxidw )  ) * F_flux ;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
dy(2) = locb + mocb_J - oxidw  - ocdeg  + 2*(mpsb - pyrw  - pyrdeg) - reductant_input ;
...
dy(3) = -locb - mocb_J + oxidw + ocdeg + ccdeg + carbw - mccb - sfw + reductant_input + C_volc;
...
dy(12) =  locb*(  delta_locb ) + mocb_J_isotope*( delta_mocb )  -   oxidw*delta_G  -   ocdeg*delta_G  ; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% dy(16) = -locb*(  delta_locb ) -mocb*( delta_mocb ) + oxidw*delta_G + ocdeg*delta_G + ccdeg*delta_C + carbw*delta_C - mccb*delta_mccb - sfw*delta_mccb + reductant_input*delta_C_volc + C_volc*delta_C_volc;

dy(16) = -locb*(delta_locb) - mocb_J_isotope*(delta_mocb) + oxidw*delta_G + ocdeg*delta_G + ccdeg*delta_C + carbw*delta_C - mccb*delta_mccb - sfw*delta_mccb + reductant_input*delta_C_volc + volc_iso_flux;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    workingstate.CO2_volc(stepnumber,1) = C_volc;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    workingstate.mocb(stepnumber,1) = mocb_J ;
    ...
    workingstate.silw(stepnumber,1) = silw_J ;
    ...
    workingstate.C_volc(stepnumber,1) = C_volc ;
